						...
						<th>
							<label class="hasTip"
								...
								<input type="checkbox" onclick="toggleDiv();"
									onchange="toggleDiv();" name="div_enable"
		...
		<script type="text/javascript">
			function modulesanywhere_jInsertEditorText(id, modulepos)
			{
				...
				if (document.getElementById('div_enable').checked) {
					...
					var params = [];
					if (f.div_width.value.trim()) {
						params[params.length] = 'width:' + f.div_width.value.trim();
					}
					if (f.div_height.value.trim()) {
						params[params.length] = 'height:' + f.div_height.value.trim();
					}
					if (float) {
						params[params.length] = 'float:' + float;
					}
					if (f.div_class.value.trim()) {
						params[params.length] = 'class:' + f.div_class.value.trim();
					}
